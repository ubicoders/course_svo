...
Viewer::Viewer(
    ...
    : map_(map), stereoCam_(stereoCam), clock_(clock),
      ...
      pathPub_(pathPub), odomPub_(odomPub), markerPub_(markerPub) {
...
void Viewer::publishCameraFrustum(const Sophus::SE3d &T_w2c) {
  ...
  double frustrum_scale = 1.0;
  double z_depth = 1; // 0.5 meter frustum depth
  ...
  double w = 1600 * frustrum_scale;
  double h = 900 * frustrum_scale;
...
void Viewer::publishTrackedPath(std::shared_ptr<Frame> frame) {
  // Accumulate tracked path
  // Rotation from Camera Frame (Z-forward, Y-down) to ROS World Frame
  // (X-forward, Z-up)
  Eigen::Matrix3d R_viz;
  R_viz << 0, 0, 1, -1, 0, 0, 0, -1, 0;

  Sophus::SE3d T_wc = frame->T_w2c.inverse();
  Eigen::Vector3d p_viz = R_viz * T_wc.translation();

  // Publish Odometry
  nav_msgs::msg::Odometry odomMsg;
  odomMsg.header.stamp = clock_->now();
  odomMsg.header.frame_id = ros2NodeConfig.output_frame_name;

  // Create rotation matrix for visualization frame
  Eigen::Matrix3d R_pose_viz = R_viz * T_wc.rotationMatrix();
  Eigen::Quaterniond q(R_pose_viz);

  odomMsg.pose.pose.position.x = p_viz.x();
  odomMsg.pose.pose.position.y = p_viz.y();
  odomMsg.pose.pose.position.z = p_viz.z();
  ...
  p.y = p_viz.y();
  p.z = p_viz.z();
  trackedPathPoints_.push_back(p);

  if (trackedPathPoints_.empty()) {
    return;
  }
  ...
  marker.points = trackedPathPoints_;
