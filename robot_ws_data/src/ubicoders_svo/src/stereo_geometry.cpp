...
cv::Mat StereoGeometry::get_T2() { return T2; }

cv::Point2f StereoGeometry::pixel2camera(const cv::Point2f &p_p) {
...
Eigen::Vector2d StereoGeometry::world2pixel(const Eigen::Vector3d &p_w,
                                            const Sophus::SE3d &T_c_w) {
  return camera2pixel(world2camera(p_w, T_c_w));
}

Eigen::Vector3d StereoGeometry::world2camera(const Eigen::Vector3d &p_w,
                                             const Sophus::SE3d &T_c_w) {
  return T_c_w * p_w;
}

Eigen::Vector2d StereoGeometry::camera2pixel(const Eigen::Vector3d &p_c) {
